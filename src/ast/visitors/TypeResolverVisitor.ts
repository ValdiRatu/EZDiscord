...
import { ArrayValue, BinaryValue, BooleanValue, FunctionCall, MathValue, NumberValue, StringValue, Variable, VarNameValue } from "../nodes";
...
export class TypeResolverVisitor extends ASTBaseVisitor<TypeResolverParam, Type>{
  ...
  visitVarNameValue(varName: VarNameValue, context: TypeResolverParam): Type {
      ...
      if (!varType) {
        context.errors.push(`invalid variable: ${chalk.blueBright(varName.value)} used out of scope or without being declared`)
        return Type.Error
      }
      return varType 
  }
